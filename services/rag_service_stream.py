...
"""
    ...
    @staticmethod
    def _bm25_rerank_candidates(query: str, docs: list, top_k: int) -> list:
        """仅对已召回候选文档做BM25排序（不改检索入口）。"""
        if not docs:
            return []
    ...
    def _apply_rerank(self, query: str, docs: list) -> list:
        """统一重排入口：先BM25排序候选，再语义重排。"""
        relevant_docs = self._bm25_rerank_candidates(query, docs, top_k=self.retrieve_k)
